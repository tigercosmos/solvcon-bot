...
void test_claude_invocation_minimal_uses_defaults()
{
    ...
    EXPECT_EQ(inv.argv.size(), static_cast<std::size_t>(8));
    ...
    EXPECT_EQ(inv.argv[5], std::string("--include-partial-messages"));
    EXPECT_EQ(inv.argv[6], std::string("--model"));
    EXPECT_EQ(inv.argv[7], std::string("claude-opus-4-8"));
...
void test_claude_invocation_model_override()
{
    ...
    EXPECT_EQ(inv.argv.size(), static_cast<std::size_t>(8));
    EXPECT_EQ(inv.argv[6], std::string("--model"));
    EXPECT_EQ(inv.argv[7], std::string("claude-sonnet-4-6"));
